...
3. Если правила требуют наказания, но этика позволяет ограничиться замечанием — выдай предупреждение (action="pardon")
...
- "pardon" — предупреждение: нарушение есть, но ограничиться замечанием без мута.
...
def format_decision_preview(decision: dict[str, Any]) -> str:
    ...
    if action == "pardon" or decision.get("punishment_type") == "warning":
