...
ЭТИЧЕСКИЕ НОРМЫ (приоритет при предупреждении вместо мута):
...
- Команда модерации по @username, если user_id цели неизвестен:
  * violator_display = "@nickname"
  * violator_user_id = user_id из УЧАСТНИКИ, если есть; иначе null и action="lookup_users"
- Если violator_user_id известен (из контекста переписки или УЧАСТНИКИ) — всегда указывай его; username не подставляй вместо id
- Если владелец/админ сам нарушает правила (оскорбления и т.д.) — оценивай по правилам чата; НЕ игнорируй автоматически. Если правила допускают снисхождение — warning или none.
...
3. Если правила требуют наказания, но этика позволяет ограничиться замечанием — выдай предупреждение (action="warning")
...
- "warning" — предупреждение: нарушение есть, но ограничиться замечанием без мута.
- "punish" — выдать мут (punishment_type="mute").
"""
...
class ModerationService:
    ...
    async def analyze(
        ...
        result = _normalize_decision(parse_moderation_response(raw))
    ...
    async def analyze_batch(
        ...
        decisions = [_normalize_decision(d) for d in parse_batch_moderation_response(raw)]
    ...
    def enrich_decision(
        ...
        result = _normalize_decision(dict(decision))
        ...
        if message.from_user and not result.get("violator_user_id"):
            ...
            elif result.get("action") in ("punish", "warning", "pardon"):
    ...
    async def finalize_decision(
        ...
        if result.get("violator_user_id"):
            return result
        ...
        if target_username:
            ...
            if participant:
                logger.info(
                    "Resolved violator by @%s -> user_id %s chat=%s",
                    target_username,
                    participant.user_id,
                    chat_id,
                )
    ...
    async def apply_decision(
        ...
        if _is_warning_decision(action, decision):
            return await self._apply_warning(
                bot,
                chat_id,
                decision,
                target_message,
        ...
        if action == "punish" and violator_id:
            ...
            ptype_label = "Мут"
            duration_str = f" на <b>{duration} мин</b>"
    ...
    async def _apply_warning(
        ...
        if not record_uid:
            logger.warning("Warning skipped: no violator chat=%s reply=%s", chat_id, reply_id)
            return None

        warning = decision.get("warning_text") or "Соблюдайте правила чата. Впредь будьте внимательнее."
        rules_str = "; ".join(rule_refs) if rule_refs else "общие нормы"
        display = record_display or str(record_uid)
        text = (
            f"⚠️ <b>Предупреждение</b>\n\n"
            f"👤 <b>{display}</b> (<code>{record_uid}</code>)\n\n"
            f"{warning}\n\n"
        ...
        await _send_chat_notice(
            ...
            reply_id,
            target_message,
            reply_markup=history_only_keyboard(warning_id),
        )
        return None
...
def format_decision_preview(decision: dict[str, Any]) -> str:
    ...
    if _is_warning_action(action, decision):
        warning = decision.get("warning_text") or "Предупреждение"
        return (
            f"⚠️ <b>Предупреждение</b>\n\n"
            f"📜 Правила: {rules_str}\n"
            f"⚠️ {warning}\n\n"
...
def _normalize_decision(decision: dict[str, Any]) -> dict[str, Any]:
    ...
    if action == "punish" and result.get("punishment_type") == "warning":
        result["action"] = "warning"
        result["punishment_type"] = None
    return result


def _is_warning_action(action: str, decision: dict[str, Any]) -> bool:
    return action in ("warning", "pardon") or (
        action == "punish" and decision.get("punishment_type") == "warning"
    )


def _is_warning_decision(action: str, decision: dict[str, Any]) -> bool:
    return _is_warning_action(action, decision)


async def _send_chat_notice(
    bot: Bot,
    chat_id: int,
    text: str,
    reply_id: int | None,
    ...
    kwargs = topic_send_kwargs(target_message)
    if reply_id:
        try:
            ...
            err = str(exc).lower()
            if "message to be replied not found" in err or "reply message not found" in err:
                logger.warning("Reply target missing chat=%s reply=%s: %s", chat_id, reply_id, exc)
            else:
                raise
    return await bot.send_message(
        chat_id,
